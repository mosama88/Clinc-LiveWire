    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title">أضافة شفت جديد</h4>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <form action="{{ route('dashboard.shiftTypes.store') }}" method="POST" role="form">
                    @csrf
                    <div class="card-body">
                        <div class="form-group">
                            <label for="">أسم الشفت</label>
                            <input class="form-control" name="name" value="{{ old('name') }}" type="text"
                                placeholder="أكتب أسم الشفت">
                            @error('name')
                        ...
                        <div class="form-group d-none">
                            ...
                            <input type="text" class="form-control font-w" name="total_hours"
                                value="{{ old('total_hours') }}" oninput="this.value=this.value.replace(/[^0-9.]/g,'');"
                                id="total_hours" placeholder="أدخل عدد الساعات">
                            @error('total_hours')
                                <div class="alert alert-danger" role="alert">
                                    {{ $message }}
                                </div>
