    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title"> تعديل التحاليل</h4>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <form action="{{ route('dashboard.testsServices.update', $info->id) }}" method="POST" role="form">
                    @csrf
                    @method('PUT')

                    <div class="card-body">
                        <div class="form-group">
                            <label for="">أسم التحاليل</label>
                            <input class="form-control" name="name" value="{{ old('name',$info['name']) }}" type="text"
                                   placeholder="أكتب أسم التحاليل">
                            @error('name')
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control font-w" name="price"
                                   ...
                                   id="price" placeholder="أدخل سعر التحاليل">
                            @error('price')
                            <div class="alert alert-danger" role="alert">
                                {{ $message }}
                            </div>
